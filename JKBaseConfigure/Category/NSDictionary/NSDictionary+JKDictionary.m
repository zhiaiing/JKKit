...
@interface _JKXMLDictionaryParser : NSObject <NSXMLParserDelegate>
@end

@implementation _JKXMLDictionaryParser {
...
- (void)textEnd {
    _text = _text.jk_trimmingWhitespaceAndNewlines.mutableCopy;
    if (_text.length) {
...
- (void)parser:(__unused NSXMLParser *)parser didEndElement:(__unused NSString *)elementName namespaceURI:(__unused NSString *)namespaceURI qualifiedName:(__unused NSString *)qName {
    ...
    if ([parent isKindOfClass:[NSArray class]]) {
        parent[[(NSArray*)parent count] - 1] = inner;
    } else {
...
- (NSArray *)jk_allValuesSortedByKeys {
    NSArray *sortedKeys = [self jk_allKeysSorted];
...
+ (NSDictionary *)jk_dictionaryWithXML:(id)xml {
    _JKXMLDictionaryParser *parser = nil;
    if ([xml isKindOfClass:[NSString class]]) {
        parser = [[_JKXMLDictionaryParser alloc] initWithString:xml];
    } else if ([xml isKindOfClass:[NSData class]]) {
        parser = [[_JKXMLDictionaryParser alloc] initWithData:xml];
    }
