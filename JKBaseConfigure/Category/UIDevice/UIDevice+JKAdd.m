...
- (NSString *)jk_machineModelName {
    static dispatch_once_t one;
    static NSString *name;
    dispatch_once(&one, ^{
        NSString *model = [self jk_machineModel];
...
- (int64_t)jk_diskSpaceUsed {
    int64_t total = self.jk_diskSpace;
    int64_t free = self.jk_diskSpaceFree;
...
- (int64_t)jk_memoryTotal {
    int64_t mem = [[NSProcessInfo processInfo] physicalMemory];
    if (mem < -1) mem = -1;
    return mem;
}

- (int64_t)jk_memoryUsed {
...
- (BOOL)jk_systemVersionNotHigherThan:(NSString *)version {
    ...
    if ([self.systemVersion isEqualToString:version]) {
        ...
        return [self jk_systemVersionLowerThan:version];
    }
}
...
- (BOOL)jk_systemVersionNotLowerThan:(NSString *)version {
    ...
    if ([self.systemVersion isEqualToString:version]) {
        ...
        return [self jk_systemVersionHigherThan:version];
    }
}

- (BOOL)jk_isJailbroken {
    if ([self jk_isSimulator]) return NO; // Dont't check simulator
    ...
    NSString *path = [NSString stringWithFormat:@"/private/%@", [NSString jk_stringWithUUID]];
    if ([@"test" writeToFile : path atomically : YES encoding : NSUTF8StringEncoding error : NULL]) {
        [[NSFileManager defaultManager] removeItemAtPath:path error:nil];
        return YES;
    }
