...
- (CGRect)jk_screenFrame {
    ...
    for (UIView *view = self; view; view = view.superview) {
        origin.x += view.jk_left;
        origin.y += view.jk_top;
    ...
    return CGRectMake(origin.x, origin.y, self.jk_width, self.jk_height);
}


- (CGFloat)jk_visibleAlpha {
...
- (void)jk_clearScrollViewDelegate {
    ...
    for (UIView *sub in self.subviews) {
        [sub jk_clearScrollViewDelegate];
    }
}
...
- (void)jk_removeAllGesturesWithSubViews {
    [self jk_removeAllGestures];
    for (UIView *v in self.subviews) {
        [v jk_removeAllGesturesWithSubViews];
    }
